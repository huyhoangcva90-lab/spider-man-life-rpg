/* ==========================================================================
   V5 NOTION SNAPSHOT REPOSITORY
   Read-only snapshot data repository with local fallback defense
   ========================================================================== */

import { NotionNormalizer } from './NotionNormalizer.js';

// Embedded fallback snapshot matching app-v5/data/notion-snapshot.json
...
export class NotionSnapshotRepository {
  ...
  async loadSnapshot() {
    try {
      ...
      console.warn('[NotionSnapshotRepository] Local fetch failed; utilizing embedded Notion snapshot.', err);
    } finally {
      this.isLoaded = true;
    }
  }
